...
import RegionHeader from 'src/views/apps/region/RegionHeader'

// ** Actions Imports
import { fetchData } from 'src/store/apps/permissions'
import { fetchRegion, updateRegion, deleteRegion } from 'src/store/apps/region/index.js'
...
const defaultColumns = [
  ...
  },
  {
    ...
    minWidth: 240,
    headerName: 'Tax',
    renderCell: ({ row }) => <Typography>{row.tax.amount}%</Typography>
  },
  {
    flex: 0.25,
    field: 'currency',
    minWidth: 240,
    headerName: 'Currency',
    renderCell: ({ row }) => <Typography>{row.currency.name}</Typography>
  },
  {
    flex: 0.25,
    minWidth: 215,
...
const Region = () => {
  ...
  const { rows, count } = useSelector(state => state.region)
  ...
  const fetchTableData = async (sort, column) => {
    ...
    let data = await dispatch(fetchRegion(q))
  }
  ...
  const handleEditPermission = (name, tax, currency, id) => {
    setformValue({ region: name, tax, currency, regionId: id })
  ...
  const onSubmit = async () => {
    let r = await(
      dispatch(
        updateRegion({
          name: formValue.region,
          currencyId: formValue.currency,
          taxId: formValue.tax,
          id: formValue.regionId
        })
      )
    )
    fetchTableData(sort, sortColumn)
    ...
    ...defaultColumns,
    {
      ...
      renderCell: ({ row }) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <IconButton onClick={() => handleEditPermission(row.name, row.tax.id, row.currency.id, row.id)}>
  ...
  return (
    <>
      <Grid container spacing={6}>
        <Grid item xs={12}>
          <PageHeader
            title={<Typography variant='h5'>Region List</Typography>}
          />
        </Grid>
        <Grid item xs={12}>
          <Card>
            <RegionHeader value={value} handleFilter={handleFilter} />
      ...
      <Dialog fullWidth maxWidth='sm' onClose={handleDialogToggle} open={editDialogOpen}>
