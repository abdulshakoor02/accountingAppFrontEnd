...
import TaxHeader from 'src/views/apps/tax/TaxHeader'

// ** Actions Imports
import { fetchData } from 'src/store/apps/permissions'
import { fetchTax, updateTax, deleteTax } from 'src/store/apps/tax/index.js'
...
const Tax = () => {
  ...
  const fetchTableData = async (sort, column) => {
    ...
    let data = await dispatch(fetchTax(q))
  ...
  const handleFilter = useCallback(val => {
    if (isNaN(val)) {
      toast.error('Tax can only be a number');
  ...
  const handleEditPermission = (amount, id) => {
    setformValue({ vat: amount, id })
  ...
  //     fetchTableData(newModel[0].sort, value, newModel[0].field)
  //   } else {
  //     setSort('asc')
  ...
  const onSubmit = async () => {
    if (isNaN(formValue.vat)) {
      toast.error('Vat can only be a number')

      return
    }

    let up = await dispatch(updateTax({ amount: parseInt(formValue.vat), id: formValue.id }))
    ...
    ...defaultColumns,
    {
      ...
      renderCell: ({ row }) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <IconButton onClick={() => handleEditPermission(row.amount, row.id)}>
  ...
  return (
    <>
      <Grid container spacing={6}>
        <Grid item xs={12}>
          <PageHeader title={<Typography variant='h5'>Tax List</Typography>} />
        </Grid>
        <Grid item xs={12}>
          <Card>
            <TaxHeader value={value} handleFilter={handleFilter} />
